...
begin
  ...
  if (I <> 0) and (I < $FF) then
    Result := Chr(Byte(I))
  else if ((Code >= $30) and (Code <= $39)) or ((Code >= $41) and (Code <= $5A))
  then
    try
...
begin
  ...
  begin
    ...
    begin
      try
        CapsOn := Lo(GetKeyState(VK_CAPITAL)) = 1;
      except
        exit;
      end;
      if eroare then
        exit;
      if CapsOn xor ShiftDown then
        try
          CharUpperBuff(@Key, 1);
        except
          exit;
      ...
      if eroare then
        exit;
      try
        PostMessage(DestWnd, CharMsg, DWORD(Key), Keycode);
      except
        exit;
      end;
      if eroare then
...
begin
  ...
  with Shared^ do
    try
      PostMessage(DestWnd, CharMsg, Keycode, Flags);
    except
      exit;
    end;
...
begin
  ...
  try
    if Code < 0 then
      try
        Result := CallNextHookEx(Shared^.KbdHook, Code, wParam, lParam);
      except
        exit;
      end;
    if eroare then
...
begin
  ...
  eroare := false;
  if Shared^.KbdHook = 0 then
  begin
    try
      Temp := SetWindowsHookEx(WH_KEYBOARD, KeyboardProc, HInstance, 0);
    except
      exit;
    end;
    if eroare then
    ...
    begin
      ...
      begin
        if eroare then
          exit;
        try
          FillChar(Shared^, SizeOf(TSharedData), #0);
          Shared^.CharMsg := RegisterWindowMessage('MTAVRE');
        except
          exit;
        end;
      end;
