...
uses {$IFDEF WIN32} CommCtrl, {$ENDIF} VCLUtils, MainColors;
...
begin
  ...
  with ABitmap.Canvas do begin
    ...
    Pen.Width := 1;
    Brush.Color := cFormForeground;
    Brush.Style := bsSolid;
    FillRect(R);
    { buttons frame }
    Pen.Color := cFormForeGround;
    Rectangle(0, 0, Width, Height);
    MoveTo(-1, Height);
    LineTo(Width, -1);
    { top button }
    if ADownState = sbTopDown then Pen.Color := cFormLine
    else Pen.Color := cFormLine;
    MoveTo(1, Height - 4);
    LineTo(1, 1);
    LineTo(Width - 3, 1);
    if ADownState = sbTopDown then Pen.Color := cFormLine
    ...
    { bottom button }
    if ADownState = sbBottomDown then Pen.Color := cFormLine
      else Pen.Color := cFormLine;
    MoveTo(2, Height - 2);
