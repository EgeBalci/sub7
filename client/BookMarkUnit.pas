...
type
  TBookmarks = class(TForm)
    CaptionLabel: TLabel;
    CloseButton: TFlatSpeedButton;
    BookmarkList: TListBox;
    AddButton: TFlatSpeedButton;
    DeleteButton: TFlatSpeedButton;
...
procedure TBookmarks.WMNCHitText(var Msg:TWMNCHitTest);
const Corner=5;
      OffSet=3;
begin
  inherited;
...
procedure TBookmarks.SetTheColors;
begin
 CaptionLabel.Font.Color:=cFormText;
 CaptionLabel.Color:=cFormCaption;
 with CloseButton do begin
  Color:=cFormCaption;ColorBorder:=cFormCaption;Font.Color:=cFormText;ColorDown:=cFormBackground;
  ColorFocused:=cFormForeground;ColorHighlight:=cFormLine;ColorShadow:=cFormLine;
 ...
 with AddButton do begin
  Color:=cFormCaption;ColorBorder:=cFormBackground;Font.Color:=cFormText;ColorDown:=cFormBackground;
  ColorFocused:=cFormForeground;ColorHighlight:=cFormLine;ColorShadow:=cFormLine;
 end;
 ...
 BookmarkList.Font.Color:=cFormText;
 BookmarkList.Color:=cFormForeground;
 Invalidate; 
end;

procedure TBookmarks.FormCreate(Sender: TObject);
begin
 SetTheColors;
 Tag:=17;Left:=(Screen.Width-Width) div 2; Top:=(Screen.Height-Height) div 2;
 ...
 try BookmarkList.Items.Text:=MainForm.ReadKey('bookmarks');except end;
end;

procedure TBookmarks.CloseButtonClick(Sender: TObject);
begin
 Hide;
end;

procedure TBookmarks.CaptionLabelMouseDown(Sender: TObject;
...
procedure TBookmarks.CaptionLabelMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TBookmarks.CaptionLabelMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
if Capturing then with CaptionLabel.Parent do begin
 ReleaseCapture;
...
procedure TBookmarks.DeleteButtonClick(Sender: TObject);
begin
 If BookmarkList.ItemIndex=-1 then exit;
  BookmarkList.Items.Delete(BookmarkList.ItemIndex);
end;

procedure TBookmarks.BookmarkListDblClick(Sender: TObject);
begin
 if BookmarkList.ItemIndex=-1 then exit;
 FileM.Path.Caption:=BookmarkList.Items[BookmarkList.ItemIndex];
 FileM.RefreshFM;
 hide;
end;

procedure TBookmarks.AddButtonClick(Sender: TObject);
var getstr:string;
begin
GetTextForm.GetText('add bookmark','new bookmark:','',false,GetStr);
If GetStr<>'' then BookmarkList.Items.Add(GetStr);
end;

procedure TBookmarks.FormPaint(Sender: TObject);
var i:integer;
begin
...
procedure TBookmarks.FormResize(Sender: TObject);
begin
 Invalidate;
end;
