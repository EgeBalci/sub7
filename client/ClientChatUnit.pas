...
procedure TClientChat.WMNCHitText(var Msg:TWMNCHitTest);
const Corner=5;
      OffSet=3;
begin
  inherited;
 with ClientChat,msg do begin
...
procedure TClientChat.SetTheColors;
begin
...
begin
 SetTheColors;
 Tag:=1;Left:=(Screen.Width-Width) div 2; Top:=(Screen.Height-Height) div 2;
...
procedure TClientChat.CloseButtonClick(Sender: TObject);
begin
 Hide;
end;

procedure TClientChat.CaptionLabelMouseDown(Sender: TObject;
...
procedure TClientChat.CaptionLabelMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TClientChat.CaptionLabelMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
if Capturing then with CaptionLabel.Parent do begin
 ReleaseCapture;
...
procedure TClientChat.OutlookBtn2Click(Sender: TObject);
begin
 MainForm.ChatSock.Socket.SendText('MTC*'+MainForm.NickName+' has left the chat.');
 MainForm.Status.Caption:='chat closed.';
 hide;
end;

procedure TClientChat.OutlookBtn1Click(Sender: TObject);
begin
 if FlatEdit1.Text<>'' then
 begin
  MainForm.ChatSock.Socket.SendText('MTC<'+MainForm.NickName+'> '+FlatEdit1.Text);
  FlatEdit1.Text:='';
 end;
end;
...
begin
 MainForm.ChatSock.Socket.SendText('CCC');
end;

procedure TClientChat.FormPaint(Sender: TObject);
var i:integer;
begin
...
procedure TClientChat.FormResize(Sender: TObject);
begin
 Invalidate;
end;
